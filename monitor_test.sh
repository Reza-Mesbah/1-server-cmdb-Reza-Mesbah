...
if [ $? -eq 0 ]; then SALDO=$((SALDO+1)); else echo "Check 7 FAILED"; fi
...
if [ $? -eq 0 ]; then SALDO=$((SALDO+1)); else echo "Check 8 FAILED"; fi
...
if [ $? -eq 0 ]; then SALDO=$((SALDO+1)); else echo "Check 9 FAILED"; fi

# Check 10 - docker in package listing
find $DOCBASE -type f 2>/dev/null | xargs grep -iq docker 2>/dev/null
if [ $? -eq 0 ]; then SALDO=$((SALDO+1)); else echo "Check 10 FAILED"; fi
